...
// Single sheet or multi-sheet controlled by mode parameter.
export function downloadXLSX(standings, catches, competition, config, mode = 'multi_sheet') {
  ...
  if (mode === 'single_sheet') {
    ...
    // Multi-sheet: generate as a combined HTML workbook
...
<h2>${escHtml(competition.name || 'Results')}</h2>
<p>${escHtml(competition.venue || '')} &nbsp;·&nbsp; ${escHtml(competition.start_date || '')}</p>
...
function buildMultiSheetHTML(standings, catches, competition, config) {
  const prizeRows = buildPrizeRows(standings, catches, config)
  // Excel multi-sheet workbook via XML Spreadsheet format
  const sheets = [
...
function standingsTable(standings) {
  ...
  const rows = standings.map(s => [
    ...
    (s.totalWeightKg || 0).toFixed(3),
    s.speciesCount || 0,
    s.catchCount || 0,
    s.bestFish?.species_name || '',
    s.bestFish?.weight_kg || '',
  ])
